...
module ZAWS
  module Command
    class Security_Group < Thor
      ...
      def view
        @aws.ec2.security_group.view(options[:region], (options[:unused] ? 'json' : options[:viewtype]), @out, (options[:verbose] ? @out : nil), options[:vpcid],nil,nil,nil,nil,nil,nil, options[:unused])
      end

      desc "exists_by_name GROUP_NAME", "Determine if a security group exists by name GROUP_NAME."
      option :vpcid, :type => :string, :desc => "AWS VPC id", :banner => "<vpcid>", :aliases => :v, :default => nil
      def exists_by_name(group_name)
        @aws.ec2.security_group.exists(options[:region], @out, (options[:verbose] ? @out : nil), options[:vpcid], group_name)
      end
      ...
      def delete(group_name)
        @aws.ec2.security_group.delete(options[:region], @out, (options[:verbose] ? @out : nil), options[:vpcid], group_name)
      end
