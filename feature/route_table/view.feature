Feature: Route Table 
  ...
  Scenario: Get route table in JSON form 
    ...
    When I run `zaws route_table view --region us-west-1 --viewtype json`
  ...
  Scenario: Get route table from specified vpcid
    ...
    When I run `zaws route_table view --region us-west-1 --vpcid my_vpc_id`
    Then the stdout should contain "AWS Route Table Output\n" 
