Feature: Route to Gateway
  ...
  Scenario: Determine a route propagation exsits by virtual gateway and route table
    Given I double `aws --output json --region us-west-1 ec2 describe-route-tables --filter 'Name=vpc-id,Values=my_vpc_id' 'Name=tag:externalid,Values=my_route_table'` with stdout:
     ...
     """
    When I run `zaws route_table propagation_exists_from_gateway my_route_table vgw-???????? --region us-west-1 --vpcid my_vpc_id`
  ...
  Scenario: Declare route propagation by gateway id and route table
	...
	And I double `aws --region us-west-1 ec2 enable-vgw-route-propagation --route-table-id rtb-???????? --gateway-id vgw-????????` with stdout:
	 ...
	 {	"return": "true" }
     """
    When I run `zaws route_table declare_propagation_from_gateway my_route_table vgw-???????? --region us-west-1 --vpcid my_vpc_id`
  ...
  Scenario: Delete route propagation from gateway.
    Given I double `aws --output json --region us-west-1 ec2 describe-route-tables --filter 'Name=vpc-id,Values=my_vpc_id' 'Name=tag:externalid,Values=my_route_table'` with stdout:
	 ...
	 {	"return": "true" }
     """
    When I run `zaws route_table delete_propagation_from_gateway my_route_table vgw-???????? --region us-west-1 --vpcid my_vpc_id`
  ...
  Scenario: Nagios OK
    Given I double `aws --output json --region us-west-1 ec2 describe-route-tables --filter 'Name=vpc-id,Values=my_vpc_id' 'Name=tag:externalid,Values=my_route_table'` with stdout:
	 ...
	 {	"RouteTables": [ { "VpcId":"my_vpc_id","RouteTableId":"rtb-????????","PropagatingVgws" :  [ { "GatewayId":"vgw-????????" } ] } ] }
     """
    When I run `zaws route_table declare_propagation_from_gateway my_route_table vgw-???????? --region us-west-1 --vpcid my_vpc_id --nagios`
...
Given I double `aws --output json --region us-west-1 ec2 describe-route-tables --filter 'Name=vpc-id,Values=my_vpc_id' 'Name=tag:externalid,Values=my_route_table'` with stdout:
  ...
  Scenario: Undo file
    ...
    When I run `zaws route_table declare_propagation_from_gateway my_route_table vgw-???????? --region us-west-1 --vpcid my_vpc_id --undofile undo.sh.1`
    Then the output should contain "Route propagation from gateway enabled.\n" 
